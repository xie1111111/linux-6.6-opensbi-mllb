...
static int forward_pass(dtype *input_data) {
    ...
    unsigned long t_out1 = __pa(&ml_shared->t_out1);
    unsigned long t_input = __pa(&ml_shared->t_input);
    unsigned long t_W1 = __pa(&ml_shared->t_W1);
    unsigned long t_B1 = __pa(&ml_shared->t_B1);
    unsigned long t_out2 = __pa(&ml_shared->t_out2);
    unsigned long t_W2 = __pa(&ml_shared->t_W2);
    unsigned long t_B2 = __pa(&ml_shared->t_B2);
    // 加锁保护共享内存访问
    ...
    // Layer 1: Matmul
    ret = matmul(t_out1, t_input, t_W1);
    if (ret.error) {
    ...
    // Layer 2: Add Bias
    ret = tensor_add(t_out1, t_out1, t_B1);
    if (ret.error) {
    ...
    // Layer 5: Add Bias
    ret = tensor_add(t_out2, t_out2, t_B2);
    if (ret.error) {
